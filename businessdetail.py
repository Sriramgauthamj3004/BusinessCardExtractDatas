...
image_url="	https://i.pinimg.com/originals/fc/71/63/fc71635c7f1b09ed30413f59bb749582.gif"
st.image(image_url, caption="Info Image", use_column_width=True)
...
selected = option_menu(None, ["Home","Upload & Extract","Modify"],
                       ...
                       styles={"nav-link": {"font-size": "25px", "text-align": "centre", "margin": "0px", "--hover-color": "#AB63FA", "transition": "color 0.3s ease, background-color 0.3s ease"},
                               ...
                               "nav-link-selected": {"background-color": "#f4beb6", "color": "white"}})
...
# HOME MENU
if selected == "Home":
        st.markdown("## :blue[**Technologies Used :**] :purple[Python] :green[EasyOCR] :purple[Streamlit] :green[SQL] :purple[Pandas]")
        st.markdown("## :blue[**Overview :**] :brown[The main purpose of this Streamlit is used to upload a Business card image,extract info with easyOCR, and manage data can able to view,modify and delete. Save entries, including images, in a versatile database.]")
    ...
    if uploaded_card is not None:
        ...
        if st.button("Upload to Database"):
            for i,row in df.iterrows():
                ...
                sql = """INSERT INTO users(company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code,image)
                                         VALUES (?,?,?,?,?,?,?,?,?,?,?)"""
...
if selected == "Modify":
    ...
    try :
        with column1:
            cursor.execute("Select card_holder FROM users")
    ...
    if st.button("View updated data"):
        cursor.execute(
            "select company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code from users")
